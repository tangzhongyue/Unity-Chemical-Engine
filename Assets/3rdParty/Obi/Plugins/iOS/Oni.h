...
namespace Oni
{
    ...
    extern "C"
    {
		...
		EXPORT void ApplyPositionInterpolation(Solver* solver,const float step_seconds);
        ...
        EXPORT void AddParticleWinds(Solver* solver, const Vector4fUnaligned* wind, int* indices, int num);
        
        EXPORT void AddParticleWind(Solver* solver, const Vector4fUnaligned& wind, int* indices, int num);
        ...
        EXPORT int GetParticleDensities(Solver* solver, float* densities, int num, int source_offset);
        
        EXPORT int GetParticleAnisotropies(Solver* solver, Vector4fUnaligned* anisotropies, int num, int source_offset);
