...
void BuildParameterSpaceMatrices(float4 t0, float4 t1, float4 t2, out float3x3 P, out float3x3 IP)
{
	...
	float3x3 O = float3x3(t0.xyz,t1.xyz,t2.xyz);
	float3x3 IO = transpose(O);
	...
	float3x3 IS = float3x3(_RadiusScale/t0.w,0,0,0,_RadiusScale/t1.w,0,0,0,_RadiusScale/t2.w);
...
{
	a2 = mul(UNITY_MATRIX_V,mul(IP,mul(IP,WorldEye(worldPos) - center))); //T^-2 * (eye - center)
	a3 = mul(UNITY_MATRIX_V,mul(IP,mul(IP,view))); 						  //T^-2 * A[0]
}
